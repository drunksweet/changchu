...
const Section = React.memo(
  ({ title, profile, name, pgithublink, content, isExpanded, onExpand }) => (
    <motion.div
    ...
    >
      <motion.div
        ...
        layout
      >
        <h2 className="text-black text-2xl font-bold p-6 border-2">{title}</h2>
        <div className="flex items-center justify-center w-full">
          <motion.img
            className="block w-32 h-32 rounded-full my-16"
            src={profile}
            alt={`${name} profile`}
